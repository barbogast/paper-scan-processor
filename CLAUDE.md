...
Wails desktop app (Go backend + React/TS frontend) for post-processing scanned PDFs. See `spec.md` for the full feature spec and `checklist.md` for implementation progress against it — check it off as steps land.
...
- `spec.md`'s prose sections are the single source of truth for behavior; `checklist.md` entries should just say what shipped (RPC/function names, implementation notes) and point back at the relevant `spec.md` section rather than re-describing the behavior.
