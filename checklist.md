...
- [x] **No caching of the authenticated Drive client** — `service()` (`backend/drive/auth.go`) now builds the authenticated `*http.Client` once and caches it in a package-level var, instead of calling `clientWithConfig` (which re-reads and re-parses `drive_token.json`) on every API call. Made safe to cache indefinitely by switching `clientWithConfig` off `oauth2.StaticTokenSource` onto a self-refreshing source (`persistingTokenSource`) that persists newly issued tokens to disk only when the access token actually changes.
...
- [x] **`PickFolder` rejection is unhandled** — `await PickFolder(...)` in `pickRoot` is still outside the try/catch, so the hook's own `loading`/`error` state doesn't reflect a dialog failure, but the concern behind this note (silent failure) no longer applies: the only call site (`DriveUploadMode`'s "Choose root folder…" button) goes through `AsyncButton`/`useAsyncAction`, which was wired up as part of item 134 in this same review pass and already catches the rejection and shows a notification.
- [x] **Stale Drive-fetch error persists after collapsing a picker row** — `DriveFolderPickerModal.tsx`'s inline error text is now gated on `expanded && error`, so collapsing a folder whose fetch failed hides the message until it's expanded again.
