...
- [ ] **Step 6d: Cancel in-flight upload** — a "Cancel" button aborts the currently uploading file (not just queued-but-not-started ones); backed by a per-job `context.WithCancel` passed through `UploadFile` down to the Drive API call; the aborted file is flagged "Cancelled" rather than left in an ambiguous "Uploading" state, since Drive may or may not have completed the upload before cancellation landed
